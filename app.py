...
# Department test
# Load your encoder
with open('data/departamento_encoder.pkl', 'rb') as f:
    departamento_encoder = pickle.load(f)
...
with st.form('user_input_form'):
    ...
    if submitted:
        ...
        input_df = pd.DataFrame({'DEPARTAMENTO': [departamento]})

        # Transform using the encoder
        encoded_df = pd.DataFrame(
            departamento_encoder.transform(input_df[['DEPARTAMENTO']]).toarray(),
            columns=departamento_encoder.get_feature_names_out(['DEPARTAMENTO'])
        )

        st.write("Dados transformados para o modelo:")
...
#         if col == 'DEPARTAMENTO':
#             user_inputs[col] = st.selectbox("Select Department", ['ANCASH', 'APURIMAC', ..., 'UCAYALI'])
#         elif col == 'PROVINCIA':
#             user_inputs[col] = st.selectbox("Select Province", ['LIMA', 'AREQUIPA', ...])
#         elif col == 'GENERO':
#             user_inputs[col] = st.selectbox("Select Gender", ['M', 'F', 'U'])
#         elif col == 'MATRICULA':
#             user_inputs[col] = st.selectbox("Select Matricula Status", ['Regular', 'Reincorporado', ...])
#         elif col == 'PAGAMENTO DE ANUIDADE MARÇO 2022':
#             user_inputs[col] = st.selectbox("Payment 2022", ['Sim', 'Não'])
#         elif col == 'PAGAMENTO DE ANUIDADE MARÇO 2023':
#             user_inputs[col] = st.selectbox("Payment 2023", ['Sim', 'Não'])
#         elif col == 'CURSO EM RISCO':
#             user_inputs[col] = st.selectbox("Curso em Risco", ['Sim', 'Não'])
#         elif col == 'CLASSIFICACAO':
#             user_inputs[col] = st.selectbox("Classification", ['Pregrado', ...])
#         elif col == 'CAMPUS':
#             user_inputs[col] = st.selectbox("Campus", ['UTP Lima Centro', ...])
#         elif col == 'FACULDADE':
#             user_inputs[col] = st.selectbox("Faculty", ['Fac. Ing.', ...])
#         elif col == 'PROGRAMA/CURSO':
#             user_inputs[col] = st.selectbox("Program/Course", ['ING. SISTEMAS', ...])
#         elif col == 'TURNO/HORARIO':
#             user_inputs[col] = st.selectbox("Shift/Schedule", ['NOCHE', 'DIA', ...])
#         elif col == 'BOLSAS DE DESCONTO':
#             user_inputs[col] = st.selectbox("Discount", ['SIN BENEFICIO', ...])
#         elif col == 'MODALIDADE DE ENSINO':
#             user_inputs[col] = st.selectbox("Teaching Mode", ['Presencial', 'Remoto', ...])
#         elif col == 'FAIXA ETARIA':
#             user_inputs[col] = st.selectbox("Age Group", ['18-20', '21-23', ...])
#         elif col == 'DEFICIENCIA':
#             user_inputs[col] = st.selectbox("Disability", ['Sim', 'Não'])
#         elif col == 'DISTRITO':
#             user_inputs[col] = st.selectbox("Select District", ['LIMA', 'SAN BORJA', ...])
#         # Add the numerical input
#     user_inputs['NUMERO DE DISCIPLINAS MATRICULADAS'] = st.number_input("Number of Disciplines", min_value=0, max_value=20, value=5)
#     user_inputs['PAGAMENTO DE ANUIDADE MARÇO 2022'] = st.selectbox("Payment 2022", ['Sim', 'Não'])
#     user_inputs['PAGAMENTO DE ANUIDADE MARÇO 2023'] = st.selectbox("Payment 2023", ['Sim', 'Não'])
